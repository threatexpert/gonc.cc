...
var (
	...
	//ipv4PortRE = regexp.MustCompile(`(^|\.)(\d+\.\d+\.\d+\.\d+)-(\d{1,5})($|\.)`)
	hostPortRE = regexp.MustCompile(`^([a-zA-Z0-9.-]+)-(\d{1,5})(?:\..+)?$`)
	tokenTxtRE = regexp.MustCompile(`^127\.(\d{1,3})\.(\d{1,3})\.(\d{1,3})(\.|$)`)
...
func (x *Xip) processQuestion(q dnsmessage.Question, srcAddr net.IP) (response Response, logMessage string, err error) {
	...
	case dnsmessage.TypeTXT:
		{
			return x.nameToTXTwithToken(q, response, logMessage)
		}
	case dnsmessage.TypeAAAA:
		{
			return x.nameToAAAAwithBlocklist(q, response, logMessage)
...
func NameToA(fqdnString string, allowPublicIPs bool) []dnsmessage.AResource {
	...
	match := hostPortRE.FindStringSubmatch(fqdnString)
	if match == nil {
		return nil
	}
	host := match[1]
...
func NameToCNAME(fqdnString string) *dnsmessage.CNAMEResource {
	...
	matches := tokenTxtRE.FindStringSubmatch(fqdnString)
	if len(matches) != 5 {
		// 不匹配，处理错误
		return nil
	}

	domainSuffix := strings.TrimPrefix(fqdnString, matches[0])

	// 提取 b.c.d
	b1, _ := strconv.Atoi(string(matches[1]))
	b2, _ := strconv.Atoi(string(matches[2]))
	b3, _ := strconv.Atoi(string(matches[3]))
	...
	// 拼成 "host-port.xxx.io"
	originalDomain := fmt.Sprintf("%s-%d.%s", entry.IP, entry.Port, domainSuffix)

	// 返回 CNAME 记录
	return &dnsmessage.CNAMEResource{
		CNAME: dnsmessage.MustNewName(originalDomain),
	}
}

func NameToTXT(fqdnString string) []dnsmessage.TXTResource {
	fqdn := []byte(fqdnString)
	match := tokenTxtRE.FindSubmatch(fqdn)
	if match == nil {
		return nil
	}

	// 提取 b.c.d
	b1, _ := strconv.Atoi(string(match[1]))
	b2, _ := strconv.Atoi(string(match[2]))
	b3, _ := strconv.Atoi(string(match[3]))
	...
	// 返回 TXT 记录
	return []dnsmessage.TXTResource{
		{TXT: []string{fmt.Sprintf("%s:%d", entry.IP, entry.Port)}},
	}
}

func (x *Xip) nameToTXTwithToken(q dnsmessage.Question, response Response, logMessage string) (_ Response, _ string, err error) {
	nameToTXTs := NameToTXT(q.Name.String())
	if len(nameToTXTs) == 0 {
		// No Answers, only 1 Authorities
		soaHeader, soaResource := SOAAuthority(q.Name)
		response.Authorities = append(response.Authorities,
	...
	x.Metrics.AnsweredQueries++
	// 如果你有专门的 TXT 统计，也可以加：
	// x.Metrics.AnsweredTXTQueries++

	response.Answers = append(response.Answers,
		func(b *dnsmessage.Builder) error {
			for _, nameToTXT := range nameToTXTs {
				err = b.TXTResource(dnsmessage.ResourceHeader{
					Name:   q.Name,
					Type:   dnsmessage.TypeTXT,
					Class:  dnsmessage.ClassINET,
	...
	var logMessages []string
	for _, nameToTXT := range nameToTXTs {
		// TXT 是 []string，这里拼起来方便日志看
		logMessages = append(logMessages, strings.Join(nameToTXT.TXT[:], " "))
	}
